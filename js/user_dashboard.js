...
import { initPublicOrgsFeature } from "./features/public_orgs.js"; 
import { initAnalyticsFeature } from "./features/analytics.js";
import { initProfileFeature } from "./features/profile.js";
...
onAuthStateChanged(auth, async (user) => {
    if (user) {
        ...
        try { if(typeof initPublicOrgsFeature === 'function') initPublicOrgsFeature(); } catch(e) { console.error("PublicOrg Err", e); }
        try { if(typeof initAnalyticsFeature === 'function') initAnalyticsFeature(); } catch(e) { console.error("Analytics Err", e); }
        try { if(typeof initProfileFeature === 'function') initProfileFeature(); } catch(e) { console.error("Profile Err", e); }
...
const views = {
    ...
    allOrgs: document.getElementById('view-all-orgs')
};
...
const navLinks = {
    ...
    allOrgs: document.getElementById('nav-all-orgs')
};
...
if(navLinks.allOrgs) navLinks.allOrgs.addEventListener('click', (e) => { e.preventDefault(); switchView('allOrgs'); });
...
async function loadUserProfile(uid) {
    try {
        ...
        if (docSnap.exists()) {
            ...
            // Image Handling
            if(data.photoUrl) {
                document.getElementById('topAvatar').src = data.photoUrl;
                document.getElementById('settingsProfileImg').src = data.photoUrl;
                document.getElementById('sidebarAvatarImg').src = data.photoUrl;
